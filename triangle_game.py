...
class Board(object):
  ...
  def display(self):
    ...
    print("Current Board")
    for i in range(5):
      print(" " * ((5 - i - 1) * indent_per_row), end="")
      for j in range(i+1):
        if self.holes[row_offset + j].peg:
          marker = "X"
        else:
          marker = "O"
        print(peg_format.format(marker), end=" " * spaces_within_row)
      print("")
      print(" " * ((5 - i - 1) * indent_per_row), end="")
      for j in range(i+1):
        print(peg_format.format(row_offset + j), end=" " * spaces_within_row)
      print("")
      print("")
      row_offset += i + 1
    return
...
while True:
  ...
  if board.game_over():
    ...
    dest = raw_input("Enter the destination position: ")
    try:
      start = int(start)
      dest = int(dest)
